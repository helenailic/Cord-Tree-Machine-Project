...
SharedPointer<T>::SharedPointer(T* address, kSharedPointerType type) {
  // 1.address of free store object that this obj will take ownership of
  holder_ = address;
  // 2.the type of the pointer being taken ownership of
  type_ = type;
  // reference count to holder_ (the address being taken care of)
...
SharedPointer<T>::SharedPointer(const SharedPointer<T>& source) {
  // increases number of SharedPointer<T> objects engaged in shared ownership of
  // source.holder_ shallow copy of source's data members since we want them to
  // be shared
